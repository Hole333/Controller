...
ApplicationWindow{
    ...
    */
    ...
    Client.Simulation{ id : simulation; }
    property bool useGamepad:false;
    property bool useSimulation:false;
    Timer{
        id:timer;
    ...
    Column{
        ...
        Rectangle{
            ...
            Rectangle{
                ...
                ZGroupBox{
                    title: qsTr("Simulation Config") + translator.emptyString;
                    width: parent.width - 15;
                    anchors.horizontalCenter: parent.horizontalCenter;
                    ...
                    Grid{
                        ...
                        property int itemWidth : (width-columnSpacing*(columns-1))/columns;
                        ZText{
                            text: qsTr("Simulate Mode");
                        }
                        Switch {
                            id : switchConrol2;
                            checked :window.useSimulation
                            onClicked: {
                                window.useSimulation = checked;
                            }
                        }
                        ZText{
                            text: qsTr("");
                        ...
                        TextField {
                            width: parent.itemWidth;
                            placeholderText: qsTr("224.5.23.2");
                            // 绑定数据到window对象，按需添加
//                            text: window.simPort || ""
                            onTextChanged: simulation.setClientAddr(text)
                        }
                        ZText{
                        ...
                        TextField {
                            width: parent.itemWidth;
                            placeholderText: qsTr("10020");
                            // 绑定数据到window对象，按需添加
//                            text: window.simPort || ""
                            onTextChanged: simulation.setClientPort(text)
                        }
                        ZText{
                            text: qsTr("grSim Address");
                        }
                        TextField {
                            width: parent.itemWidth;
                            ...
                            // 绑定数据到window对象，按需添加
//                            text: window.simPort || ""
                            onTextChanged: simulation.setGrSimAddr(text)
                        }
                        ZText{
                            text: qsTr("grSim Port");
                        }
                        TextField {
                            width: parent.itemWidth;
                            placeholderText: qsTr("20011");
                            // 绑定数据到window对象，按需添加
//                            text: window.simPort || ""
                            onTextChanged: simulation.setGrSimAddr(text)
                        }
                ...
                ZGroupBox{
                    ...
                    Grid{
                        ...
                        ZText{ text:"Sides" }
                        ComboBox{
                            model:simulation.getSidesSetting();
                            currentIndex : 1;
                            onActivated: simulation.getControlSide(index);
                            width:parent.itemWidth;
                        }
